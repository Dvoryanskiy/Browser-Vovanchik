...
// На некоторых системах webview рендерится некорректно (частичный экран/артефакты).
// Отключаем аппаратное ускорение для стабильного отображения.
  ...
  try {
    ...
    mainWindow = new BrowserWindow({
      ...
      webPreferences: {
        ...
        webSecurity: false // Временно отключаем безопасность для диагностики
      }
    });
      ...
      submenu: [
        {
          ...
          accelerator: 'F11',
          click: () => {
            // mainWindow.setFullScreen(!mainWindow.isFullScreen()); // Временно отключено для отладки масштабирования webview
          }
        }
      ]
    }
